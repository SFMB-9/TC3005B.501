...
la solicitud de compra de un usuario Comprador. Utilizado 
cuando se carga la pagina de solicitud de compra de usuario 
Comprador.
...
export default async function handler(req, res) {
    ...
    if(req.method === 'GET' && req.body !== null) {
        try {
            ...
            return res
                .status(200)
                .json({ message: 'Documentos de compra recuperado exitosamente', result: result });
        } catch(err) {
            return res.status(400).json({ message: 'Error al recuperar documentos de compra', error: err});
        }
    }
}
